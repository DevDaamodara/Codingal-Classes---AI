...
def similarity(a: str, b: str) -> float:
    ...
    if not r.ok: raise RuntimeError(f"API error {r.status_code}: {r.text}")
    data = r.json()
...
def verdict(score: float, threshold: float) -> str:
    if score >= threshold:        return "🔴 TOO SIMILAR (possible copy)"
    if score >= threshold - 0.15: return "🟡 KIND OF SIMILAR"
...
def main():
    ...
    while True:
        ...
        if mode == "demo":
            for i, (a1, a2) in enumerate(random.sample(PAIRS, 3), 1):
                s   = similarity(a1, a2)
                pct = round(s * 100, 1)
                print(f"\n[{i}]  Answer 1: {a1}\n     Answer 2: {a2}")
            ...
            a1 = input("Student Answer 1: ").strip()
